...
async function handleAdminUI(user) {
  const db = firebase.firestore();
  const userRef = db.collection("users").doc(user.uid);
  ...
  const superAdminElements = document.querySelectorAll(".admin-super-only");
  // Reset visibility
  adminElements.forEach(el => el.classList.remove("admin-visible"));
  superAdminElements.forEach(el => el.classList.remove("admin-visible"));

// Admin (but NOT superadmin stuff)
  if (role === "admin") {
    adminElements.forEach(el => {
      if (!el.classList.contains("admin-super-only")) {
  ...
  if (role === "superadmin") {
    adminElements.forEach(el => el.classList.add("admin-visible"));
    superAdminElements.forEach(el => el.classList.add("admin-visible"));
  }
