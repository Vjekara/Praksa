(function () {
  ...
  async function createPlantEntryInFirestore(plantPayload) {
    ...
    try {
      ...
      await entryRef.set({
        createdAt: new Date().toLocaleTimeString("hr-HR", {
      ...
      date: new Date().toLocaleDateString("hr-HR", {
        ...
        metadata: {
          ...
          plantId: plantPayload.catalogPlantId || null,
          temp: plantPayload.temperature || null
        },

        type: plantPayload.name || ""
      });

    } catch (err) {
      console.error("Failed creating Firestore entry:", err);
    }
  }
  ...
  async function loadPlantCatalog() {
    ...
    try {
      const snap = await firebase.firestore().collection("plants").get();
      ...
      snap.forEach(doc => {
        ...
        if (metadata.naziv) {
          plantCatalog.push({
            id: doc.id,
            name: metadata.naziv,
            strain: metadata.sorta || ""
          });
        }
      });

      plantCatalog.sort((a, b) => a.name.localeCompare(b.name));

    } catch (err) {
      console.error("Failed to load plant catalog:", err);
    }
  }

  function populatePlantCatalogSelect(selectedValue = "") {
  ...
  function attachPlantCatalogAutoFill() {
    const select = document.getElementById("plant-name");
    const strainInput = document.getElementById("plant-strain");

    if (!select || !strainInput) return;

    select.onchange = () => {
      const selected = plantCatalog.find(
        plant => plant.name === select.value
      );

      strainInput.value = selected ? selected.strain : "";
    };
  }
  ...
  function openPlantModal(editId) {
    ...
    populatePlantCatalogSelect();
    attachPlantCatalogAutoFill();
    ...
    if (editId) {
      ...
      if (p) {
        ...
        populatePlantCatalogSelect(p.name);
  ...
  document.getElementById('form-plant').addEventListener('submit', (e) => {
    ...
    const payload = {
      ...
      catalogPlantId: (() => {
        const selected = plantCatalog.find(
          p => p.name === document.getElementById('plant-name').value.trim()
        );
    ...
    setPlants(next);
    if (!id) {
      createPlantEntryInFirestore(payload);
    }
