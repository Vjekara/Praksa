...
function renderRow(data) {
  ...
  if (page === "entries") {
    return `${data.type || "-"} - ${data.ownerUid || "-"}`;
  }
...
addBtn.onclick = () => {
  editId = null;
  const template = structuredClone(schemas[page]);
  openModal(template);
};
  ...
  modal.innerHTML = `
    <div class="modal-content">
      ...
      <div style="margin-top:16px; display:flex; gap:10px;">
        ...
        <button id="cancelDynamic">Cancel</button>
      </div>
    </div>
  `;

  document.getElementById("cancelDynamic").onclick = () => {
    modal.classList.remove("open");
    modal.innerHTML = "";
  };

  modal.querySelectorAll(".fill-now-btn").forEach(btn => {
    btn.onclick = () => {
      const target = btn.dataset.target;
      const input = modal.querySelector(`[data-key="${target}"]`);
        ...
        <div style="margin-bottom:10px;">
          <div style="display:flex; gap:8px; align-items:center;">
            ...
            ${
              ...
              <button
                type="button"
                class="fill-now-btn btn btn-ghost"
                data-target="${fullKey}"
              >
                Now
              </button>
